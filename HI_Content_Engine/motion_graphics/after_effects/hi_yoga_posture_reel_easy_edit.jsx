/*
  hi_yoga_posture_reel_easy_edit.jsx

  Easiest version of the HI posture reel builder.
  Edit only the EASY_EDIT block below, then run the script in After Effects.

  You do not need to touch the rest of the file unless you want deeper customization.
*/

(function buildEasyHiYogaPostureReel() {
    if (!app.project) {
        app.newProject();
    }

    app.beginUndoGroup("Build Easy HI Yoga Posture Reel");
    ...
    var TARGET_LIBRARY = {
        ...
        SHOULDERS: { ctrlName: "GLOW_SHOULDERS_CTRL", position: [450, 940], glowScale: [150, 95] },
    ...
    var REEL_CONFIG = buildConfig(EASY_EDIT);

    function buildConfig(easyEdit) {
        var bubbles = [];
        var controlPoints = [];
        var seenControls = {};
        var i;

        for (i = 0; i < easyEdit.cues.length; i += 1) {
            var cue = easyEdit.cues[i];
            var targetKey = cue.target;

            if (targetKey === "FULL_BODY") {
                bubbles.push({
                    name: cueName(i + 1),
                    start: cue.start,
                    end: cue.end,
                    text: cue.text,
                    bubblePos: DEFAULT_BUBBLE_POSITIONS[i] || [790, 980],
                    targetCtrl: TARGET_LIBRARY.FULL_BODY_SPINE.ctrlName,
                    fullBodyOverlay: true
                });

                registerControl(TARGET_LIBRARY.FULL_BODY_HANDS);
                registerControl(TARGET_LIBRARY.FULL_BODY_SPINE);
                registerControl(TARGET_LIBRARY.FULL_BODY_HIPS);
                registerControl(TARGET_LIBRARY.FULL_BODY_HEELS);
            } else {
                var targetData = TARGET_LIBRARY[targetKey];
                bubbles.push({
                    name: cueName(i + 1),
                    start: cue.start,
                    end: cue.end,
                    text: cue.text,
                    bubblePos: DEFAULT_BUBBLE_POSITIONS[i] || [790, 980],
                    targetCtrl: targetData.ctrlName,
                    glowTarget: targetData.ctrlName,
                    glowScale: targetData.glowScale
                });
                registerControl(targetData);
            }
        }

        function registerControl(targetData) {
            if (seenControls[targetData.ctrlName]) {
                return;
            }
            seenControls[targetData.ctrlName] = true;
            controlPoints.push({ name: targetData.ctrlName, position: targetData.position });
        }
        ...
        return {
            ...
            duration: 60,
            fps: 30,
            compName: easyEdit.compPrefix + "__MASTER",
            postureName: easyEdit.postureName,
            faceSafeZone: {
                position: [540, 300],
                size: [360, 340]
            },
            ...
            bubbles: bubbles,
            centerMoments: [
                { name: "OPENING_BREATH", start: 0, end: 5, text: easyEdit.openingText },
                { name: "EXHALE_RESET", start: 40, end: 45, text: easyEdit.exhaleText },
                { name: "STAY_FOR_FIVE", start: 50, end: 55, text: easyEdit.stayText, withBreathCircle: true }
            ],
            closingCta: {
                start: 55,
                end: 60,
                text: easyEdit.ctaText,
                showLogo: easyEdit.showLogo
            }
        };
    }
    ...
    function createThoughtBubbleTemplate(parentFolder) {
        var comp = ensureComp("THOUGHT_BUBBLE__TEMPLATE__EASY", 640, 220, 1, 8, REEL_CONFIG.fps, parentFolder);
        clearComp(comp);
    ...
    function createGlowTemplate(parentFolder) {
        var comp = ensureComp("GLOW_HIGHLIGHT__TEMPLATE__EASY", 340, 340, 1, 8, REEL_CONFIG.fps, parentFolder);
        clearComp(comp);
    ...
    function createBreathCircleTemplate(parentFolder) {
        var comp = ensureComp("BREATH_CIRCLE__TEMPLATE__EASY", 500, 500, 1, 8, REEL_CONFIG.fps, parentFolder);
        clearComp(comp);
        var shapeLayer = comp.layers.addShape();
    ...
    function createClosingCtaTemplate(parentFolder) {
        var comp = ensureComp("CLOSING_CTA__TEMPLATE__EASY", 1080, 420, 1, 8, REEL_CONFIG.fps, parentFolder);
        clearComp(comp);
    ...
    function createFullBodyOverlayTemplate(parentFolder, mainCompName) {
        var comp = ensureComp("FULL_BODY_CONNECTION__TEMPLATE__EASY", REEL_CONFIG.width, REEL_CONFIG.height, 1, 8, REEL_CONFIG.fps, parentFolder);
        clearComp(comp);
    ...
    function addBubbleInstance(mainComp, bubbleTemplate, cue, cueFolder) {
        var cueComp = duplicateCompWithName(bubbleTemplate, cue.name + "__BUBBLE__EASY", cueFolder);
    ...
    var easyFolder = ensureFolder("After_Effects_Templates__Easy_Edit", rootFolder);
    var moduleFolder = ensureFolder("HI_Modules__Easy", easyFolder);
    var cueFolder = ensureFolder("HI_Cue_Comps__Easy", easyFolder);
    ...
    var mainComp = ensureComp(REEL_CONFIG.compName, REEL_CONFIG.width, REEL_CONFIG.height, 1, REEL_CONFIG.duration, REEL_CONFIG.fps, easyFolder);
    clearComp(mainComp);
    ...
    alert(
        "Easy HI posture reel template created.\n\n" +
        "Edit only the EASY_EDIT block next time.\n" +
